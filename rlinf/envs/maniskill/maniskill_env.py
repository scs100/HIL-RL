...
class ManiskillEnv(gym.Env):
    def __init__(
        ...
        # 🔥 键盘HIL支持
        self._use_keyboard_hil = cfg.get("use_keyboard_hil", False)
        if self._use_keyboard_hil:
            ...
            self._keyboard_expert = KeyboardIntervention(None, intervention_window)
            self._keyboard_expert.action_dim = self.action_space.shape[0]
            print(f"🎮 ManiSkill环境已启用键盘HIL (干预窗口: {intervention_window}秒)")
